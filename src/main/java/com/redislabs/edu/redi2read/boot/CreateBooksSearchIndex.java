    ...
    public void run(String... args) throws Exception {
        ...
        try {
            ...
            if (rcee.getMessage().equals("Unknown Index name")) {
                ...
                Field<String> author0 = Field.tag("authors.[0]").build();
                Field<String> author1 = Field.tag("authors.[1]").build();
                Field<String> author2 = Field.tag("authors.[2]").build();
                Field<String> author3 = Field.tag("authors.[3]").build();
                Field<String> author4 = Field.tag("authors.[4]").build();
                Field<String> author5 = Field.tag("authors.[5]").build();
                Field<String> author6 = Field.tag("authors.[6]").build();

                commands.create(searchIndexName, options,
                        title, subtitle, description,
                        author0, author1, author2,
                        author3, author4, author5,
                        author6);
                log.info(">>> Created RediSearch index: {}", searchIndexName);
            }
        }
    }
}
